...
class Attribute extends BaseModel {
    ...
    public static function get_goods_attr_info($arr, $type = 'pice')
    {
        ...
        if (!empty($arr))
        {
            ...
            $res = GoodsAttr::selectRaw('attr_price,attr_value,attr_name as name,attr_type')->whereIn('goods_attr_id',$arr)->leftJoin('attribute',function ($query) use($arr){
     ...
     * @return      array
     */
    public static function get_goods_attr_info_byid( $goodid )
    {
        ...
        $res = GoodsAttr::selectRaw('attr_price as info,attr_value,attr_name as name,attr_type')->where('goods_id',$goodid)->leftJoin('attribute',function ($query) use($arr){
            $query->on('attribute.attr_id','=','goods_attr.attr_id');
        })->get();
        foreach ($res as $key => $row) {
